...
export class JobsService {
  ...
  getJobsFromRemoteWWR() {
    ...
    this.httpService.post<CreateJobDto[]>(`https://uolblavtiyb2ugq6ocvhaphsxm0bkbfs.lambda-url.us-east-2.on.aws/`)
      .pipe(take(1))
      .subscribe({
        next: (res) => {
  ...
  getJobsFromRemoteOK() {
    ...
    this.httpService.post<CreateJobDto[]>(`https://lqpebweraqrf5hfaznu3jsccti0iekjs.lambda-url.us-east-2.on.aws/`)
      .pipe(take(1))
      .subscribe({
        next: res => {
          this.createMultiple(res.data, { user: { userId: 1 } } as any)
  ...
  async createMultiple(jobs: CreateJobDto[], req: any) {
    console.log('Creating Multiple Jobs', jobs.length)
    ...
    const filteredJobs: CreateJobDto[] = jobs.filter((job) => !existingJobs.includes(job.customId))
